...
static int line_type = LineSolid;
void SetDrawMotif(char dash_list[],int n)
{

  	if (display == NULL) return;
	if (n)
		{	
  	XSetDashes(display,drawgc,0,dash_list,n);
  	XSetLineAttributes(display, drawgc, line_width, LineOnOffDash, CapButt, JoinBevel); 
	line_type = LineOnOffDash; 
		}
	else
		{
  	XSetLineAttributes(display, drawgc, line_width, LineSolid, CapButt, JoinBevel); 
	line_type =LineSolid ;
		}
 }
...
{
  if (display && width > 0)
    XSetLineAttributes(display, drawgc, width, line_type, CapButt, JoinBevel);
	line_width = width;
}
